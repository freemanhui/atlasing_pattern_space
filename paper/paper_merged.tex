...
models, and present experimental validation on MNIST demonstrating that
...
maintaining reconstruction quality. The framework's applicability extends
...
Experimental results (Section~\ref{experiments}) demonstrate that TopologyEnergy
...
data-generating factors. Our MNIST experiments demonstrated that APS yields latent
maps where \textbf{neighborhoods are meaningful (T)}, \textbf{axes align
with stable concepts (C)}, and \textbf{energy wells reinforce rather than
compete with geometric structure (E)}. These properties improve both
