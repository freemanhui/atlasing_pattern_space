...
\textbf{Energy as Training Regularizer:} The TopologyEnergy component effectively prevents overfitting \emph{regardless of shift strength}. However, its OOD accuracy improvements are negligible (+0.11pp, likely statistical noise), suggesting energy-based constraints primarily serve as capacity control mechanisms rather than direct OOD generalization tools.
...
TopologyEnergy's effectiveness in preventing representation collapse on MNIST (902\% ARI improvement) while maintaining training stability on AG News validates its data-driven design: energy wells emerge from neighborhood structure rather than arbitrary memory patterns, making it robust across modalities.
